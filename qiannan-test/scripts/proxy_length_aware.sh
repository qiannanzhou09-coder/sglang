...
DYNAMIC_CACHE_HIT_THRESHOLD="${DYNAMIC_CACHE_HIT_THRESHOLD:-}"
DYNAMIC_HIGH_CACHE_HIT="${DYNAMIC_HIGH_CACHE_HIT:-}"
DYNAMIC_LOW_KV_USAGE="${DYNAMIC_LOW_KV_USAGE:-}"
DYNAMIC_HIGH_KV_USAGE="${DYNAMIC_HIGH_KV_USAGE:-}"
...
SESSION_PAUSE_MIN_SECS="${SESSION_PAUSE_MIN_SECS:-}"
SERVER_METRICS_URL="${SERVER_METRICS_URL:-}"
SERVER_METRICS_INTERVAL="${SERVER_METRICS_INTERVAL:-}"
...
if [[ -n "${DYNAMIC_CACHE_HIT_THRESHOLD}" ]]; then
  PROXY_ARGS+=(--dynamic-cache-hit-threshold "${DYNAMIC_CACHE_HIT_THRESHOLD}")
fi
...
if [[ -n "${DYNAMIC_LOW_KV_USAGE}" ]]; then
  PROXY_ARGS+=(--dynamic-low-kv-usage "${DYNAMIC_LOW_KV_USAGE}")
fi

if [[ -n "${DYNAMIC_HIGH_KV_USAGE}" ]]; then
  PROXY_ARGS+=(--dynamic-high-kv-usage "${DYNAMIC_HIGH_KV_USAGE}")
fi
...
if [[ -n "${SESSION_PAUSE_MIN_SECS}" ]]; then
  PROXY_ARGS+=(--session-pause-min-secs "${SESSION_PAUSE_MIN_SECS}")
fi

if [[ -n "${SERVER_METRICS_URL}" ]]; then
  PROXY_ARGS+=(--server-metrics-url "${SERVER_METRICS_URL}")
fi

if [[ -n "${SERVER_METRICS_INTERVAL}" ]]; then
  PROXY_ARGS+=(--server-metrics-interval "${SERVER_METRICS_INTERVAL}")
fi
